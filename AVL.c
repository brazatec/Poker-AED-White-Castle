	...
	if(*raiz==NULL){
		...
		*raiz = novo;
		...
		if(valor < atual->info){
            ...
            atual->count++;      // só soma uma ocorrência
            return 1;
            }
		}
...
int remover(ArvAVL* raiz, int valor){	
	int res;
	if(*raiz==NULL){ // Valor nao encontrado!
	...
	if(valor < (*raiz)->info){
		...
		if(((*raiz)->esq == NULL) || ((*raiz)->dir == NULL)){ // O Pai tem 1 ou nenhum filho
			...
			free(noVelho);
			return 1;
		}else{ // o no tem 2 filhos: precisamos substituir pelo no mais a esquerda da subarvore da direita
